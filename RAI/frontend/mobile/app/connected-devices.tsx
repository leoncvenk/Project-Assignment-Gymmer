  ...
  useEffect(() => {
    ...
    async function loadDevices() {
      try {
        ...
        if (!token) {
          console.log('Ni auth tokena.');
          setDevices([]);
          return;
        }

        // Ob odprtju strani registriramo trenutno napravo kot aktivno
        await registerCurrentPhone(token);

        // Nato naložimo vse naprave trenutnega userja
        await refreshDevices(token);

        // Heartbeat vsakih 30 sekund
