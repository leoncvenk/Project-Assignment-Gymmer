...
import { Calendar, Dumbbell, Clock, Flame, Share2, MoreHorizontal, Trash2, User } from 'lucide-react';
...
export default function ActivityFeed({ activities, userData, onDelete, onViewDetails }) {
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-xs font-semibold text-gray-800 uppercase tracking-wider mb-2">Recent Activities</h3>
          ...
          >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-sm font-semibold text-[#2b2b2b]">
                    {userData?.username || "You"} <span className="font-normal text-gray-500 text-xs">logged a workout</span>
                  </h4>
            ...
            <div className="px-4 pb-2">
              <h3 className="text-base font-semibold mb-2 flex items-center gap-2">
                {activity.title}
                <span className="text-lg" title="Feeling">{activity.feeling}</span>
              </h3>
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                ...
                {activity.calories > 0 && (
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-[#f2f2f2] border border-gray-200 text-xs font-medium text-gray-700">
                    <Flame className="w-3.5 h-3.5 text-[#ef4444]" /> {activity.calories} kcal
                  </span>
                )}
              </div>
            ...
            {activity.image && (
              <div className="w-full h-64 bg-gray-100 border-y border-gray-100 overflow-hidden relative group">
                <img src={activity.image} alt="Workout" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
              </div>
            )}
