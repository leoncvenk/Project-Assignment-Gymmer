          ...
          <AnimatePresence>
            {currentMealData?.entries?.length > 0 ? (
              currentMealData.entries.map((entry, idx) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-[#2b2b2b] group-hover:text-[#00a97f] transition-colors">{entry.food?.name}</p>
                      <div className="flex items-center gap-2 mt-0.5">
                        <p className="text-[11px] text-gray-500 font-medium">{entry.quantity_g}g</p>
                        <span className="w-1 h-1 rounded-full bg-gray-300"></span>
                        <p className="text-[10px] text-gray-400">
                          P: {Math.round(entry.food?.protein_100g * (entry.quantity_g/100))}g • 
                          C: {Math.round(entry.food?.carbs_100g * (entry.quantity_g/100))}g • 
                          F: {Math.round(entry.food?.fat_100g * (entry.quantity_g/100))}g
                        </p>
                      </div>
                    </div>
                  </div>
                  <span className="font-bold text-sm text-[#2b2b2b]">
                    {Math.round(entry.calories)} <span className="text-xs font-normal text-gray-500">kcal</span>
                  </span>
                </motion.div>
              ))
            ) : (
              <div className="h-full min-h-[150px] flex flex-col items-center justify-center text-gray-400 bg-gray-50/50 rounded-xl border border-dashed border-gray-200">
                ...
                <p className="text-xs font-medium">No food logged for {selectedMealType} yet.</p>
              </div>
            )}
          </AnimatePresence>
        </div>
